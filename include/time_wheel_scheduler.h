#ifndef TIME_WHEEL_SCHEDULER
#define TIME_WHEEL_SCHEDULER
...
public:
  ...
  uint32_t CreateTimerAt(int64_t when_ms, const TimerTask_t& handler);
  uint32_t CreateTimerAfter(int64_t delay_ms, const TimerTask_t& handler);
  uint32_t CreateTimerEvery(int64_t interval_ms, const TimerTask_t& handler);
