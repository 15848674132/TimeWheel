...
uint32_t TimeWheelScheduler::CreateTimerAt(int64_t when_ms, const TimerTask_t &handler)
{
    if (time_wheels_.empty())
    {
        return 0;
...
uint32_t TimeWheelScheduler::CreateTimerAfter(int64_t delay_ms, const TimerTask_t &handler)
{
    int64_t when = GetNowTimestamp() + delay_ms;
    return CreateTimerAt(when, handler);
}

uint32_t TimeWheelScheduler::CreateTimerEvery(int64_t interval_ms, const TimerTask_t &handler)
{
    if (time_wheels_.empty())
    {
        return 0;
